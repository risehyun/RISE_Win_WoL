...
void ITEM_DaggerofMidas::Start()
{
	// 아이템 텍스처 로딩
	if (false == ResourcesManager::GetInst().IsLoadTexture("ITEM_DaggerofMidas.bmp"))
	{
	...
	if (false == ResourcesManager::GetInst().IsLoadTexture("ITEM_Descript_DaggerofMidas.bmp"))
	{
		GameEnginePath FilePath;
		FilePath.SetCurrentPath();
		FilePath.MoveParentToExistsChild("ContentsResources");
		...
		ResourcesManager::GetInst().TextureLoad(FilePath.PlusFilePath("ITEM_Descript_DaggerofMidas.bmp"));

	}

	// 렌더러 생성과 로딩된 텍스처 지정
	MainRenderer = CreateRenderer();
	MainRenderer->SetTexture("ITEM_DaggerofMidas.bmp");
...
void ITEM_DaggerofMidas::Update(float _Delta)
{
